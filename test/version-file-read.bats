...
@test "fails without arguments" {
  run zigenv-version-file-read
  assert_failure
  refute_output
}

@test "fails for invalid file" {
  run zigenv-version-file-read "non-existent"
  assert_failure
  refute_output
}
...
@test "fails for blank file" {
  echo > my-version
  run zigenv-version-file-read my-version
  assert_failure
  refute_output
}
...
@test "prevents directory traversal" {
  ...
  run zigenv-version-file-read my-version
  assert_failure
  assert_output "zigenv: invalid version in \`my-version'"
